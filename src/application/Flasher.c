...
void FL_vWrite(WriteCommand_t* command, SL_ResponsePrototype_t* response)
{
    ...
    {
        NAK->response = eNAK,
        NAK->reason = eInvalidState;
        return; 
    }

    //Check if write goes beyond memory range
    if(((UINT16_MAX - command->length) < flasherState.pc) || (command->length > sizeof(command->data)))
    {
        NAK->response = eNAK,
        NAK->reason = eInvalidLength;
        return; 
    }
...
void FL_vRead(ReadCommand_t* command, SL_ResponsePrototype_t* response)
{
    ...
    if(((UINT16_MAX - command->length) < flasherState.pc) || (command->length > sizeof(ACK->data)))
    {
        NAK->response = eNAK,
        NAK->reason = eInvalidLength;
        return; 
...
void FL_vWriteAt(WriteAtCommand_t* command, SL_ResponsePrototype_t* response)
{
    ...
    //Check if write goes beyond memory range
    if(((UINT16_MAX - command->length) < command->pc) || (command->length > sizeof(command->data)))
    {
        NAK->response = eNAK,
        NAK->reason = eInvalidLength;
        return; 
...
void FL_vReadFrom(ReadFromCommand_t* command, SL_ResponsePrototype_t* response)
{
    ...
    //Check if read goes beyond memory range
    if(((UINT16_MAX - command->length) < command->pc) || (command->length > sizeof(ACK->data)))
    {
        NAK->response = eNAK,
        NAK->reason = eInvalidLength;
        return; 
...
static void FL_vWriteByteToBus(uint16_t address, uint8_t byte)
{
    ...
    //Bring Memory lines to base state for write
    ...
    //Set Data Bus
    dataPortValue = 0;
    dataPortValue |= (LL_GPIO_ReadOutputPort(DATA_PORT) & 0xFFFFFF00); //Store state, LSByte used for data bus
    dataPortValue |= byte;
...
static uint8_t FL_uReadByteFromBus(uint16_t address)
{
    ...
    //Read Data Pins
    uint8_t data = (LL_GPIO_ReadInputPort(DATA_PORT) & 0xFF); //Read only the lowest byte
